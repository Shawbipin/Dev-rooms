  ...
  try {
    ...
    if (skills)
      profileObj.skills = skills
        ...
        .map((skill) => skill.trim()); // method removes whitespace from both sides of a string.
    ...
    // populate : profile can use  "user schema" or get details of user.
...
router.put("/", authenticate, async (request, response) => {
  try {
    ...
    // what ever the change made by user is gonna saved in profileobje
    let profileObj = {}; // new updated profile
    profileObj.user = request.user.id; // id gets from Token
    if (company) profileObj.company = company;
    else profileObj.company = "";
    if (website) profileObj.website = website;
    ...
    profile = await Profile.findOneAndUpdate(
      ...
      { new: true } 
      // { new: true }: This option specifies that you want to return the updated document 
      // as the result of the findOneAndUpdate operation. If you don't include this option or set
      // it to false, the method would return the document as it was before the update.
    );
...
router.put(
  ...
  async (request, response) => {
    try {
      ...
      profile.experience.unshift(newExperience); //  adding newExperience from front
      profile = await profile.save(); // need to save in write operation
      response.status(200).json({ profile: profile });
    } catch (error) {
      console.error(error);
      response.status(500).json({ errors: [{ msg: error.message }] });
...
router.delete("/experience/:expId", authenticate, async (request, response) => {
  try { 
    ...
    //  profile.experience.map((exp) => exp._id.toString())  --- it gives array of string 
    // _id is object that why it converted to string 
