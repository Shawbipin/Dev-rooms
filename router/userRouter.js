...
const bcrypt = require("bcryptjs"); // encrypted password
...
router.post( "/login",async (request, response) => {
    try {
      ...
      // check if the correct email and user existance
      let user = await User.findOne({ email: email });
      ...
      if (!user) {
        return response
          ...
          // with json it store value in data 
      }
      // Encryption -->> is the process by which a readable message 
      // is converted to an unreadable form to prevent unauthorized parties from reading it. 
      // Decryption--->> is the process of converting an encrypted message back 
      // to its original (readable) format. The original message is called the plaintext message.


      // check the passwords
      let isMatch = await bcrypt.compare(password, user.password);
      // password -- is given by user 
      // user.password-- is store in database
      if (!isMatch) {
        return response
          .status(201)
          .json({ errors: [{ msg: "Invalid Credentials" }] });
      ...
      /* This code is creating a JSON Web Token (JWT) for the authenticated user and 
      sending it back to the client as a response to the login request. The `jwt.sign()` method takes in a payload
      ...
      //**************  what will parameter taken by  jwt token -- header and secret key 
