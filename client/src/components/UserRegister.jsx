...
let UserRegister = () => {
  ...
  let submitRegistration = async (event) => {
    ...
    //  check whether the user has entered valid values for the name, email,
    // and password fields before attempting to submit the registration form
    if (

      // The trim() method is a built-in JavaScript string method 
      // that removes whitespace (spaces, tabs, and line breaks) 
      // from both the beginning and end of a string. It does not
      //  remove spaces within the string, only leading and trailing spaces.
      user.name.trim() !== "" &&
      user.email.trim() !== "" &&
      user.password.trim() !== ""
    ) {
      ...
      // await keyword is used here because making an 
      // HTTP request is an asynchronous operation, and 
      // it means that we want to wait for the request to 
      // complete before proceeding further.
      const { status } = await axios.post(
        "https://devroom-backend-8m7a.onrender.com/api/users/register",
        { name, email, password },
        {
          headers: {
